...
module.exports = async function (req, res) {
  ...
  try {
    ...
    if (!response.ok) throw new Error(`Cin7 Document API returned status ${response.status}`);
    
    const data = await response.json();
    return res.status(200).json(data);
  } catch (error) {
    return res.status(500).json({ error: "Failed to fetch document payload", details: error.message });
  }
};
